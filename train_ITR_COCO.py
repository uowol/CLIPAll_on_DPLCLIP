...
class CLIPALL(CLIP):
    ...
    def update(self, minibatch):        # [0]: image, [1]: caption
        ...
        x = minibatch[0].to(self.device).cuda().float()                             # [batch_size, 224, 224, 3]
        y = clip.tokenize(minibatch[1]).to(self.device).cuda().long()               # [batch_size, 77]
        target = torch.tensor(list(range(x.shape[0]))).to(self.device).cuda().long()
...
for epoch in range(1, n_epoch+1):
    ...
    for minibatches in train_dataloader:
